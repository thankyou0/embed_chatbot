...
async def database_exception_handler(request: Request, exc: SQLAlchemyError):
    ...
    if "password authentication failed" in error_msg.lower():
        ...
        detail = (
            "Unable to connect to database. Please verify your database credentials."
        )
    elif (
        ...
        detail = (
            "Unable to reach the database server. Please ensure PostgreSQL is running."
        )
    elif "does not exist" in error_msg.lower():
        user_msg = "Database not found"
        detail = (
            "The specified database does not exist. Please run migrations to create it."
        )
...
async def general_exception_handler(request: Request, exc: Exception):
    ...
    public_detail = sanitize_error_message(
        str(exc), fallback="An unexpected error occurred"
    )
    response_content = {
        "success": False,
...
async def health_check():
    ...
    return {
        ...
        "service": "chatbot-api",
        "redis": (
            "connected" if is_redis_available() else "unavailable (using fallback)"
        ),
    }
