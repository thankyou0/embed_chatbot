...
from app.models.subscription import (
    ...
    BillingCycle as DBBillingCycle,
...
class BillingService:
    ...
    async def calculate_current_usage(
        db: AsyncSession, tenant_id: int, period_start: datetime, period_end: datetime
    ) -> CurrentUsage:
        """Calculate current usage for tenant in the given period - OPTIMIZED"""

        # Get subscription for global message count
        subscription = await BillingService.get_or_create_subscription(db, tenant_id)

        # OPTIMIZATION: Use CTE to get chatbot IDs once
        chatbot_ids_cte = (
            select(Chatbot.id)
            .where(and_(Chatbot.tenant_id == tenant_id, Chatbot.deleted_at.is_(None)))
            .cte("tenant_chatbots")
        )

        # OPTIMIZATION: Single query to get chatbot count
        chatbots_count_result = await db.execute(
            select(func.count()).select_from(chatbot_ids_cte)
        )
        chatbots_count = chatbots_count_result.scalar() or 0

        if chatbots_count == 0:
            # No chatbots, return zeros
            return CurrentUsage(
                chatbots_count=0,
                messages_count=0,
        ...
        # OPTIMIZATION: Single query for conversations count
        conversations_count_result = await db.execute(
            select(func.count(ChatSession.id)).where(
                and_(
                    ChatSession.chatbot_id.in_(select(chatbot_ids_cte)),
        ...
        # OPTIMIZATION: Single query for messages count with JOIN
        ...
        # OPTIMIZATION: Knowledge sources CTE
        ks_ids_cte = (
            select(KnowledgeSource.id)
            .where(KnowledgeSource.chatbot_id.in_(select(chatbot_ids_cte)))
            .cte("tenant_knowledge_sources")
        )

        # OPTIMIZATION: Single aggregation query for pages, files, storage
        knowledge_agg_result = await db.execute(
            select(
                func.count(CrawledPage.id).label("pages_count"),
                func.count(UploadedFile.id).label("files_count"),
                func.coalesce(func.sum(UploadedFile.file_size), 0).label(
                    "total_storage"
                ),
            )
            .select_from(ks_ids_cte)
            .outerjoin(
                CrawledPage,
                and_(
                    CrawledPage.knowledge_source_id == ks_ids_cte.c.id,
                    CrawledPage.is_removed == False,
                ),
            )
            .outerjoin(
                UploadedFile, UploadedFile.knowledge_source_id == ks_ids_cte.c.id
            )
        )
        knowledge_agg = knowledge_agg_result.one()
        knowledge_pages_count = knowledge_agg.pages_count or 0
        knowledge_files_count = knowledge_agg.files_count or 0
        storage_bytes = knowledge_agg.total_storage or 0
    ...
    async def get_billing_overview(
        ...
        usage_percentages = {
            "chatbots": (
                (current_usage.chatbots_count / current_plan.limits.chatbots * 100)
                if current_plan.limits.chatbots > 0
                else 0
            ),
            "messages": (
                (
                    current_usage.global_message_count
                    / current_plan.limits.messages_per_month
                    * 100
                )
                if current_plan.limits.messages_per_month > 0
                else 0
            ),
            "conversations": (
                (
                    current_usage.conversations_count
                    / current_plan.limits.conversations_per_month
                    * 100
                )
                if current_plan.limits.conversations_per_month > 0
                else 0
            ),
            "knowledge_pages": (
                (
                    current_usage.knowledge_pages_count
                    / current_plan.limits.knowledge_pages
                    * 100
                )
                if current_plan.limits.knowledge_pages > 0
                else 0
            ),
            "knowledge_files": (
                (
                    current_usage.knowledge_files_count
                    / current_plan.limits.knowledge_files
                    * 100
                )
                if current_plan.limits.knowledge_files > 0
                else 0
            ),
            "team_members": (
                (
                    current_usage.team_members_count
                    / current_plan.limits.team_members
                    * 100
                )
                if current_plan.limits.team_members > 0
                else 0
            ),
            "storage": (
                (float(current_usage.storage_mb) / current_plan.limits.storage_mb * 100)
                if current_plan.limits.storage_mb > 0
                else 0
            ),
        }
    ...
    async def change_plan(
        db: AsyncSession, tenant_id: int, user: User, request: ChangePlanRequest
    ) -> ChangePlanResponse:
        ...
        if request.new_plan == PlanType.FREE:
            ...
            if current_usage.chatbots_count > new_limits.chatbots:
                errors.append(
                    f"You have {current_usage.chatbots_count} chatbots but Free plan allows only {new_limits.chatbots}"
                )
            if current_usage.team_members_count > new_limits.team_members:
                errors.append(
                    f"You have {current_usage.team_members_count} team members but Free plan allows only {new_limits.team_members}"
                )
        ...
        subscription.billing_cycle = (
            DBBillingCycle(request.billing_cycle.value)
            if request.new_plan != PlanType.FREE
            else None
        )
        ...
        amount = (
            plan_config.pricing.annual_price
            if request.billing_cycle == BillingCycle.ANNUAL
            else plan_config.pricing.monthly_price
        )
        ...
        logger.info(
            f"Tenant {tenant_id} changed plan from {old_plan} to {request.new_plan.value}"
        )
    ...
    async def check_file_upload_limit(
        db: AsyncSession, tenant_id: int, chatbot_id: str
    ) -> dict:
        """
    ...
    async def check_storage_limit(
        db: AsyncSession, tenant_id: int, additional_bytes: int = 0
    ) -> dict:
        """
