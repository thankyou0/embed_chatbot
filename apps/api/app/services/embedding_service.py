    ...
    for attempt in range(max_retries):
        try:
            ...
            result = await loop.run_in_executor(
                None, lambda: client.feature_extraction(text=texts, model=model)
            )
            ...
            for item in result:
                if isinstance(item, list) and len(item) > 0:
                    ...
                    embeddings.append(
                        list(item) if hasattr(item, "__iter__") else [item]
                    )

            logger.info(
                f"Successfully got {len(embeddings)} embeddings from HuggingFace"
            )
            return embeddings
            ...
            is_timeout = any(
                ...
                for keyword in ["504", "gateway timeout", "timeout", "timed out"]
            )
            ...
            if is_timeout and attempt < max_retries - 1:
                ...
                logger.warning(
                    f"HuggingFace API timeout (attempt {attempt + 1}/{max_retries}). Retrying in {delay}s..."
                )
...
class EmbeddingService:
    @staticmethod
    def chunk_text(
        text: str, max_tokens: int = 512, overlap: int = 50, min_tokens: int = 100
    ) -> List[str]:
        """
    ...
    async def process_knowledge_source(knowledge_source_id: UUID):
        ...
        async with session_factory() as db:
            try:
                ...
                if not ks:
                    logger.error(
                        f"Knowledge source {knowledge_source_id} not found for embedding"
                    )
                    return
                ...
                await db.execute(
                    delete(Embedding).where(Embedding.knowledge_source_id == ks.id)
                )
                await db.commit()

                # Invalidate query cache for this chatbot (knowledge changed → cached answers may be stale)
                try:
                    from app.services.cache_service import invalidate_chatbot_cache

                    await invalidate_chatbot_cache(str(ks.chatbot_id))
                except Exception as cache_err:
                    logger.warning(
                        f"Cache invalidation failed (non-critical): {cache_err}"
                    )
                ...
                for page in pages:
                    ...
                    for i, chunk in enumerate(chunks):
                        ...
                        has_product_data = (
                            hasattr(page, "is_product") and page.is_product
                        ) or (
                        ...
                        if (
                            has_product_data
                            and hasattr(page, "product_metadata")
                            and page.product_metadata
                        ):
                ...
                for qa in qa_pairs:
                    ...
                    all_metadata.append({"qa_id": str(qa.id), "type": "qa_pair"})

                if not all_chunks:
                    logger.warning(
                        f"No content chunks generated for KS: {knowledge_source_id}"
                    )
                    await db.execute(
                        update(KnowledgeSource)
                        .where(KnowledgeSource.id == knowledge_source_id)
                ...
                for batch_start in range(0, total_chunks, EMBEDDING_BATCH_SIZE):
                    ...
                    for chunk_text, vector, meta in zip(
                        batch_chunks, batch_embeddings, batch_metadata
                    ):
                        emb_obj = Embedding(
                            chatbot_id=ks.chatbot_id,
                    ...
                    logger.info(
                        f"Batch {batch_start // EMBEDDING_BATCH_SIZE + 1}: Stored {total_stored}/{total_chunks} embeddings"
                    )
                ...
                logger.success(
                    f"Successfully processed and stored {total_stored} embeddings for KS: {ks.id}"
                )
                ...
                public_error = sanitize_error_message(
                    raw_error, fallback="Embedding generation failed. Please try again."
                )
                ...
                try:
                    ...
                    logger.error(
                        f"Updated knowledge source {knowledge_source_id} status to FAILED due to embedding error"
                    )
    ...
    async def process_single_qa_pair(qa_id: UUID):
        ...
        async with session_factory() as db:
            try:
                ...
                # Get KS for metadata
                stmt_ks = select(KnowledgeSource).where(
    ...
    async def cleanup_removed_pages_embeddings(knowledge_source_id: UUID):
        ...
        async with session_factory() as db:
            try:
                ...
                if not removed_urls:
                    logger.info(
                        f"No removed pages to clean up embeddings for KS: {knowledge_source_id}"
                    )
                    return
                ...
                await db.commit()
                logger.info(
                    f"Cleaned up {deleted_count} embeddings for {len(removed_urls)} removed pages from KS: {knowledge_source_id}"
                )
